...
export class PermissionService {
  ...
  async createPermission(createPermissionDto: CreatePermissionDto): Promise<ReadPermissionDto> {
    const { name, description } = createPermissionDto;

    const permission = new Permission(name, description);

    const createPermission = await this.permissionRepository.createPermission(permission);
  ...
  async updatePermission(dto: UpdatePermissionDto): Promise<ReadPermissionDto> {
    ...
    const updatePermission = await this.permissionRepository.updatePermission(permission);
