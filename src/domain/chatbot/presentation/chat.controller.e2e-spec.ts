...
describe('ChatController (e2e)', () => {
  ...
  describe('GET - /chat/bot', () => {
    it('채팅 목록 조회하기', async () => {
      ...
      const { statusCode, body } = await request(app.getHttpServer())
        .get(`/chat?page=1&take=10&orderby=name&direction=desc&where__type=TELEGRAM`)
        .set('Cookie', su);
    ...
    it('타입 없이 목록 조회', async () => {
      ...
      expect(statusCode).toEqual(400);
      expect(body.message).toEqual(formatMessage(ExceptionEnum.PARAMETER_NOT_FOUND, { param: "type" }));
    });
