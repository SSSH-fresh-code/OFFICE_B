...
describe('ChatBotController (e2e)', () => {
  ...
  describe('PUT - /chat/bot', () => {
    it('챗봇을 수정합니다.', async () => {
      ...
      const createdBot = await prismaService.chatBot.create({ data: { ...dto } });
    ...
    it('채팅 아이디 목록을 추가합니다.', async () => {
      ...
      const createdBot = await prismaService.chatBot.create({ data: { ...dto }, include: { chats: true } });
  ...
  describe('GET - /chat/bot', () => {
    ...
    it('챗봇 목록 조회하기 - type으로 조회', async () => {
      ...
      const { statusCode, body } = await request(app.getHttpServer())
        .get(`/chat/bot?page=1&take=10&orderby=name&direction=desc&where__type=DISCORD`)
        .set('Cookie', su);
