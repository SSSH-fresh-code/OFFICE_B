...
import { POST_REPOSITORY, POST_SERVICE, SERIES_REPOSITORY, TOPIC_REPOSITORY } from "../../blog.const";
...
describe('PostSerivce', () => {
  ...
  beforeAll(async () => {
    ...
    seriesRepository = module.get<jest.Mocked<SeriesRepository>>(SERIES_REPOSITORY);
  ...
  describe('createPost', () => {
    it('게시글을 생성합니다.', async () => {
      ...
      expect(result).toEqual(post.toDto());
      expect(repository.save).toHaveBeenCalledWith(new Post(
        0, dto.title, dto.content,
  ...
  describe('updatePost', () => {
    it('title을 수정합니다.', async () => {
      ...
      repository.findById.mockResolvedValue(new Post(
        post.id,
        post.title,
        post.content,
        post.author,
    ...
    it('기존 내용과 같으면 에러를 반환합니다.', async () => {
      ...
      await expect(() => postService.updatePost(dto)).rejects.toThrow(ExceptionEnum.NOT_MODIFIED);
    });
