...
import { POST_REPOSITORY, SERIES_REPOSITORY, TOPIC_REPOSITORY } from "../../blog.const";
...
export class PostService implements iPostService {
  constructor(
    ...
    @Inject(SERIES_REPOSITORY) private readonly seriesRepository: SeriesRepository,
  ...
  async getPosts(dto: PagingPostDto): Promise<Page<ReadPostDto>> {
    ...
    return {
      data: readPosts, total: posts.total
    };
  };
  ...
  async updatePost(dto: UpdatePostDto): Promise<ReadPostDto> {
    ...
    if (dto.authorName && post.author.name !== dto.authorName) {
      const userForUpdate = await this.userRepository.findByName(dto.authorName);
    ...
    if (
      ...
      if (!dto.seriesId && post.series) { // 삭제인 경우
        post.series = null;
      } else {
        const seriesForUpdate = await this.seriesRepository.findById(dto.seriesId);
    ...
    if (isUpdated) {
      ...
      throw new SsshException(ExceptionEnum.NOT_MODIFIED, HttpStatus.BAD_REQUEST);
    }
  };
