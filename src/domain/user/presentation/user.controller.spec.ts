...
describe("UserController", () => {
	...
	describe("createUser", () => {
		...
		it("잘못된 파라미터 값이 주어지면 예외를 던져야 합니다.", async () => {
			...
			userService.createUser.mockRejectedValue(
				new HttpException("유저를 생성할 수 없습니다.", HttpStatus.BAD_REQUEST),
			);
	...
	describe("updateUserName", () => {
		it("유저를 수정해야 합니다.", async () => {
			...
			expect(await userController.updateUserName(updatedUser)).toEqual(
				updatedUser,
			);
		});
		...
		it("존재하지 않는 유저 ID가 주어지면 예외를 던져야 합니다.", async () => {
			...
			userService.updateUserName.mockRejectedValue(
				new HttpException("유저를 찾을 수 없습니다.", HttpStatus.NOT_FOUND),
			);

			await expect(userController.updateUserName(updatedUser)).rejects.toThrow(
				"유저를 찾을 수 없습니다.",
			);
		});
	});
	...
	describe("getUsers", () => {
		it("유저 목록을 조회해야 합니다.", async () => {
			...
			expect(await userController.getUsers(pagingDto)).toEqual({
				data: users,
				total,
			});
		});
	...
	describe("getUserById", () => {
		...
		it("존재하지 않는 유저 ID가 주어지면 예외를 던져야 합니다.", async () => {
			...
			await expect(userController.getUserById(id)).rejects.toThrow(
				"유저를 찾을 수 없습니다.",
			);
		});
	});
	...
	describe("updateUserPermission", () => {
		it("유저 id로 조회 후 권한을 수정합니다.", async () => {
			...
			const mockUser = new User("1", "email@example.com", "password", "name", [
				"LOGIN001",
			]);
			...
			expect(userService.updateUserPermission).toHaveBeenCalledWith(
				mockUser.id,
