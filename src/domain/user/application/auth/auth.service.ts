...
export class AuthService {
  ...
  async validateUser(email: string, password: string): Promise<any> {
    ...
    if (user) {
      const hasLoginPermission = user.permissions.includes(PermissionEnum.CAN_LOGIN);

      if (!hasLoginPermission) {
        throw new SsshException(ExceptionEnum.ACCOUNT_WITHOUT_PERMISSION, HttpStatus.FORBIDDEN);
