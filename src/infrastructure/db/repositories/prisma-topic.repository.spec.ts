...
describe('PrismaTopicRepository', () => {
  ...
  beforeAll(async () => {
    const module: TestingModule = await Test.createTestingModule({
      ...
      providers: [PrismaService, PrismaTopicRepository, ConfigService],
    }).compile();
  ...
  describe('save', () => {
    ...
    it('이미 존재하는 이름으로 Topic을 생성합니다.', async () => {
      ...
      await expect(() => repository.save(dummyTopic)).rejects.toThrow(
        Prisma.PrismaClientKnownRequestError,
      );
    });
  });
  describe('update', () => {
    ...
    it('이미 존재하는 이름으로 Topic을 수정합니다.', async () => {
      ...
      await expect(() => repository.update(t)).rejects.toThrow(
        Prisma.PrismaClientKnownRequestError,
      );
    });

    it('존재하지 않는 Topic을 수정합니다.', async () => {
      await expect(() => repository.update(dummyTopic)).rejects.toThrow(
        Prisma.PrismaClientKnownRequestError,
      );
    });
  });
  ...
  describe('delete', () => {
    ...
    it('존재하지 않는 Topic을 삭제합니다.', async () => {
      await expect(() => repository.delete(dummyTopic.id)).rejects.toThrow(
        Prisma.PrismaClientKnownRequestError,
      );
    });
  });
