...
export class PermissionGuard implements CanActivate {
	...
	canActivate(context: ExecutionContext): boolean {
		const requiredPermissionInClass =
			this.reflector.getAllAndOverride<string[]>("permissions-class", [
				context.getHandler(),
				context.getClass(),
			]) ?? [];
		const requiredPermissionInMethod =
			this.reflector.getAllAndOverride<string[]>("permissions-method", [
				context.getHandler(),
				context.getClass(),
			]) ?? [];

		const requiredPermission = [
			...requiredPermissionInClass,
			...requiredPermissionInMethod,
		];
		...
		const hasRole = user.permissions.some((permission: string) =>
