...
describe('PermissionGuard', () => {
  ...
  it('로그인하지 않은 경우 예외를 던져야 합니다.', () => {
    ...
    const context = mockExecutionContext(null);
    expect(() => guard.canActivate(context)).toThrow(new SsshException(ExceptionEnum.NOT_LOGGED_IN, HttpStatus.FORBIDDEN));
  });
  ...
  it('권한이 없는 경우 예외를 던져야 합니다.', () => {
    ...
    expect(() => guard.canActivate(context)).toThrow(new ForbiddenException(ExceptionEnum.FORBIDDEN));
  });
  ...
  it('필요한 권한이 있는 경우 통과되어야 합니다.', () => {
    ...
    const context = mockExecutionContext({ permissions: ['required_permission'] });
    expect(guard.canActivate(context)).toBe(true);
  });
});